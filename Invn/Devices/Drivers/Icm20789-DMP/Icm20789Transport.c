...
#include "Icm20789Transport.h"
#include "Icm20789Serif.h"
#include "Icm20789Defs.h"
#include "Icm20789.h"

int inv_icm20789_read_reg(struct inv_icm20789 * s, uint8_t reg,	uint8_t * buf, uint32_t len)
{
	return inv_icm20789_serif_read_reg(&s->serif, reg, buf, len);
}

int inv_icm20789_write_reg(struct inv_icm20789 * s, uint8_t reg, const uint8_t * buf, uint32_t len)
{
	return inv_icm20789_serif_write_reg(&s->serif, reg, buf, len);
}
...
int inv_icm20789_mems_write_reg(struct inv_icm20789 * s, uint16_t reg, uint32_t length, const uint8_t *data)
{
    int result = 0;
	uint32_t bytesWrite = 0;
    const uint8_t regOnly = (uint8_t)(reg & 0x7F);

    const uint8_t orig_power_state = inv_icm20789_get_chip_power_state(s);
	...
	if((orig_power_state & CHIP_AWAKE) == 0) {
		...
		if(check_reg_access_lp_disable(reg))    // Check if register needs to be awake
			result = inv_icm20789_set_chip_power_state(s, CHIP_AWAKE, 1);
	}

	if((orig_power_state & CHIP_LP_ENABLE) != 0) {
		if(check_reg_access_lp_disable(reg))    // Check if register needs LP_EN to be disabled
			result |= inv_icm20789_set_chip_power_state(s, CHIP_LP_ENABLE, 0);   // Disable LP_EN
	}
	...
	while (bytesWrite<length)
	{
		...
		result |= inv_icm20789_write_reg(s, regOnly+bytesWrite, &data[bytesWrite], thisLen);
	...
	if((orig_power_state & CHIP_LP_ENABLE) != 0) {
		if(check_reg_access_lp_disable(reg))    // Check if register needs LP_EN to be disabled
			result |= inv_icm20789_set_chip_power_state(s, CHIP_LP_ENABLE, 1);   // Enable LP_EN
	}

    // Put the chip back to sleep if it was asleep originally
	if((orig_power_state & CHIP_AWAKE) == 0) {
		if(check_reg_access_lp_disable(reg))    // Check if register needs to be awake
			result |= inv_icm20789_set_chip_power_state(s, CHIP_AWAKE, 0);
	}

	return result;
}

int inv_icm20789_mems_write_reg_one(struct inv_icm20789 * s, uint16_t reg, uint8_t data)
{
	const uint8_t regOnly = (uint8_t)(reg & 0x7F);

	return inv_icm20789_write_reg(s, regOnly, &data, 1);
}

int inv_icm20789_mems_read_reg(struct inv_icm20789 * s, uint16_t reg, uint32_t length, uint8_t *data)
{
	int result = 0;
	...
	while(bytesRead<length) {
		...
		result |= inv_icm20789_read_reg(s, regOnly+bytesRead, &data[bytesRead], thisLen);
...
int inv_icm20789_read_mems(struct inv_icm20789 * s, unsigned short reg, unsigned int length, unsigned char *data)
{
	int result=0;
    ...
    power_state = inv_icm20789_get_chip_power_state(s);
	if((power_state & CHIP_AWAKE) == 0)   // Wake up chip since it is asleep
		result = inv_icm20789_set_chip_power_state(s, CHIP_AWAKE, 1);

	lBankSelected = (reg >> 8);
	if (lBankSelected != s->lLastBankSelected) {
		result |= inv_icm20789_write_reg(s, REG_MEM_BANK_SEL, &lBankSelected, 1);
		if (result)
			return result;
	...
	while (bytesWritten < length) {
		...
		result |= inv_icm20789_write_reg(s, REG_MEM_START_ADDR, &lStartAddrSelected, 1);
		if (result)
			return result;
		...
		if(s->base_state.serial_interface == SERIAL_INTERFACE_SPI) {
			result |= inv_icm20789_read_reg(s, REG_MEM_R_W, &dat[bytesWritten], thisLen);
		} else {
			result |= inv_icm20789_read_reg(s, REG_MEM_R_W, &data[bytesWritten], thisLen);
		}
		if (result)
			return result;
...
int inv_icm20789_write_mems(struct inv_icm20789 * s, unsigned short reg, unsigned int length, const unsigned char *data)
{
    int result=0;
    unsigned int bytesWritten = 0;
    unsigned int thisLen;
    ...
    while (bytesWritten < length) {
        ...
        /* Write data */ 
        result |= inv_icm20789_write_reg(s, REG_MEM_R_W, &data[bytesWritten], thisLen);
        if (result)
            return result;
