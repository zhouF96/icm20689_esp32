...
/** @defgroup DriverIcm20789 Icm20789 driver
 *  @brief    Low-level driver for Icm20789 devices
 *  @ingroup  Drivers
 *  @{
 */

#ifndef _INV_ICM20789_H_
#define _INV_ICM20789_H_
...
/** @brief Icm20789 driver states definition
 */
typedef struct inv_icm20789 {
	struct inv_icm20789_serif serif;
	struct inv_icm20789_serif serif_ois;
	inv_bool_t use_serif_ois; /* set to 1 is ois serif has been set */

	/** @brief icm20789 internal state structure*/
	struct inv_icm20789_states {
		uint8_t  wake_state;
		uint8_t  pwr_mgmt_1;
		uint8_t  pwr_mgmt_2;
		uint8_t  lp_config;
	...
	uint8_t  power_state;
	uint32_t enabled_sensor_mask; // a bit mask corresponding to enum inv_icm20789_sensor
	uint16_t requested_odr[INV_ICM20789_SENSOR_MAX];
	uint8_t  dropped_data[INV_ICM20789_SENSOR_MAX];
 ...
 */
extern void inv_icm20789_sleep(int ms);
 ...
 */
extern uint64_t inv_icm20789_get_time_us(void);
 ...
 */
extern uint64_t inv_icm20789_get_dataready_interrupt_time_us(void);
 ...
 */
static inline void inv_icm20789_reset_states(struct inv_icm20789 * s,
		const struct inv_icm20789_serif * serif)
{
	memset(s, 0, sizeof(*s));
	s->serif = *serif;
}
 ...
 *  Must be called after inv_icm20789_reset_states()
 *  @param[in] s handle to driver states structure
 *  @param[in] serif handle to SERIF object for underlying register access to OIS
 */
static inline void inv_icm20789_reset_states_serif_ois(struct inv_icm20789 * s,
        const struct inv_icm20789_serif * serif_ois)
{
	if(serif_ois) {
