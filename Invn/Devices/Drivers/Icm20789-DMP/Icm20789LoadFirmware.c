...
//#include "Icm20789DataBaseDriver.h"

int inv_icm20789_firmware_load(struct inv_icm20789 * s, const unsigned char *data_start, unsigned short size_start, unsigned short load_addr)
{ 
    int write_size;
    int result;
	...
	inv_icm20789_get_whoami(s, &whoami);	
	dmp_icm20789_get_sw_key_addr(s, &keyaddr);
    ...
    while (size > 0) {
		...
		if( (memaddr<=keyaddr) && ((memaddr+write_size)>=keyaddr) ) {
	        ...
	        result = inv_icm20789_write_mems(s, memaddr, write_size, data_key);
		} else {		
	        result = inv_icm20789_write_mems(s, memaddr, write_size, (unsigned char *)data);
		}
        if (result)  
            return result;
        data += write_size;
    ...
    while (size > 0) {
        ...
        result = inv_icm20789_read_mems(s, memaddr, write_size, data_cmp);
        if (result)
