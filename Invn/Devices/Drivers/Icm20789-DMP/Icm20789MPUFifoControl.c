...
//#include "Icm20789DataBaseControl.h"
...
int inv_icm20789_identify_interrupt(struct inv_icm20789 * s, short *int_read)
{
    ...
    result = inv_icm20789_mems_read_reg(s, REG_INT_STATUS, 1, &int_status);
    if(int_read)
        *int_read = int_status;

    result = inv_icm20789_mems_read_reg(s, REG_DMP_INT_STATUS, 1, &int_status); // DMP_INT_STATUS
	if(int_read)
		*int_read |= (int_status << 8);
    
    /*if(wake_on_motion_enabled) {
        result = inv_icm20789_read_mems_reg(s, REG_INT_STATUS, 1, &int_status);//INT_STATUS
        if(result)
            return result;
        *int_read |= reg_data[1];
    }*/
     ...
     */
    //result = inv_icm20789_read_mems_reg(s, 0x1B, 1, &int_status);
...
**/
static int dmp_get_fifo_length(struct inv_icm20789 * s, uint_fast16_t * len )
{
	...
	result = inv_icm20789_mems_read_reg(s, REG_FIFO_COUNT_H, 2, fifoBuf);
	if (result) 
	{
...
*/
static int dmp_reset_fifo(struct inv_icm20789 * s)
{
	...
	{ 
		...
		result |= inv_icm20789_mems_write_reg(s, REG_USER_CTRL, 1, &s->base_state.user_ctrl);
		inv_icm20789_sleep(5);
		
		data = BIT_FIFO_RST;
        result |= inv_icm20789_mems_write_reg(s, REG_USER_CTRL, 1, &data);
		data = BIT_DMP_RST;
        result |= inv_icm20789_mems_write_reg(s, REG_USER_CTRL, 1, &data);
...
*  @return MPU_SUCCESS or non-zero error code
**/
static int dmp_read_fifo(struct inv_icm20789 * s, unsigned char *data, uint_fast16_t len)
{
	int result;
    ...
    while (bytesRead<len) 
    {
        ...
        result = inv_icm20789_mems_read_reg(s, REG_FIFO_R_W, thisLen, &data[bytesRead]);
        if (result)
		{
			dmp_reset_fifo(s);
			s->fifo_info.fifoError = -1;
...
**/
static uint_fast16_t dmp_get_fifo_all(struct inv_icm20789 * s, uint_fast16_t length, unsigned char *buffer, int *reset)
{
	int result;
...
*/
static int extract_sample_cnt(struct inv_icm20789 * s, int fifo_size, unsigned short * total_sample_cnt, unsigned short * sample_cnt_array)
{
...
int inv_icm20789_fifo_swmirror(struct inv_icm20789 * s, int *fifo_sw_size, unsigned short * total_sample_cnt, unsigned short * sample_cnt_array)
{
	int reset=0; 
...
int inv_icm20789_fifo_pop(struct inv_icm20789 * s, unsigned short *user_header, unsigned short *user_header2, int *fifo_sw_size)  
{
	...
	if (*fifo_sw_size > 3) {
		...
		fifo_ptr += inv_icm20789_inv_decode_one_fifo_packet(s, &fd, fifo_ptr);        
...
#if 0
int inv_icm20789_dmp_process_fifo(struct inv_icm20789 * s, int *left_in_fifo, unsigned short *user_header, unsigned short *user_header2, long *time_stamp)  
{
	...
	// no need to extract number of sample per sensor for current function, so provide 0 as last parameter
        ...
        //time stamp 
        ts = inv_icm20789_get_tick_count();
...
*/
int inv_icm20789_inv_decode_one_fifo_packet(struct inv_icm20789 * s, struct inv_fifo_decoded_t *fd, const unsigned char *fifo_ptr)
{
...
int inv_icm20789_dmp_get_accel(long acl[3])
{
    if(!acl) return -1;
...
int inv_icm20789_dmp_get_raw_gyro(short raw_gyro[3])
{
...
int inv_icm20789_dmp_get_gyro_bias(long gyro_bias[3])
{
...
int inv_icm20789_dmp_get_calibrated_gyro(long cal_gyro[3])
{
...
int inv_icm20789_dmp_get_6quaternion(long quat[3])
{
    if(!quat) return -1;
...
int inv_icm20789_dmp_get_9quaternion(long quat[3])
{
    if(!quat) return -1;
...
int inv_icm20789_dmp_get_raw_compass(long raw_compass[3])
{
...
int inv_icm20789_dmp_get_calibrated_compass(long cal_compass[3])
{
...
int inv_icm20789_dmp_get_bac_state(uint16_t *bac_state)
{
...
int inv_icm20789_dmp_get_bac_ts(long *bac_ts)
{
	if(!bac_ts) return -1;
	*bac_ts = fd.bac_ts;
	return 0;
...
int inv_icm20789_dmp_get_flip_pickup_state(uint16_t *flip_pickup)
{
...
*/
int inv_icm20789_get_accel_accuracy(void)
{
...
*/
int inv_icm20789_get_gyro_accuracy(void)
{
