...
#include "Icm20789Ctrl.h"
#include "Icm20789Defs.h"
#include "Icm20789.h"
#include "Icm20789MPUFifoControl.h"
...
#define INV_NEEDS_ACCEL_MASK    (/*1L<<INV_ICM20789_SENSOR_ACCELEROMETER | */\
                                 1L<<INV_ICM20789_SENSOR_RAW_ACCELEROMETER | \
								 1L<<INV_ICM20789_SENSOR_GAME_ROTATION_VECTOR)
								  
#define INV_NEEDS_GYRO_MASK     (1L<<INV_ICM20789_SENSOR_GYROSCOPE | \
                                 1L<<INV_ICM20789_SENSOR_RAW_GYROSCOPE | \
								 1L<<INV_ICM20789_SENSOR_GYROSCOPE_UNCALIBRATED | \
								 1L<<INV_ICM20789_SENSOR_GAME_ROTATION_VECTOR)
...
/* Convert public sensor type for Icm20789 to internal sensor id (Android)
 */
/*
static uint8_t sensor_type_2_android_sensor(enum inv_icm20789_sensor sensor)
{
	switch(sensor) {
	case INV_ICM20789_SENSOR_ACCELEROMETER:                 return ANDROID_SENSOR_ACCELEROMETER;
	case INV_ICM20789_SENSOR_GYROSCOPE:                     return ANDROID_SENSOR_GYROSCOPE;
	case INV_ICM20789_SENSOR_RAW_ACCELEROMETER:             return ANDROID_SENSOR_RAW_ACCELEROMETER;
	case INV_ICM20789_SENSOR_RAW_GYROSCOPE:                 return ANDROID_SENSOR_RAW_GYROSCOPE;
	case INV_ICM20789_SENSOR_GYROSCOPE_UNCALIBRATED:        return ANDROID_SENSOR_GYROSCOPE_UNCALIBRATED;
	case INV_ICM20789_SENSOR_GAME_ROTATION_VECTOR:          return ANDROID_SENSOR_GAME_ROTATION_VECTOR;
	default:                                                return ANDROID_SENSOR_NUM_MAX;	
	}
}
*/

static uint32_t is_icm20789_sensor_enabled(struct inv_icm20789 * s, uint8_t icm20789_sensor)
{
	return (s->enabled_sensor_mask & (1L << icm20789_sensor));
}

static int apply_accel_fsr(struct inv_icm20789 * s)
{
	int result = 0;
	uint8_t accel_config_1_reg;

	// Set FSR
	result |= inv_icm20789_mems_read_reg(s, MPUREG_ACCEL_CONFIG, 1, &accel_config_1_reg);
	accel_config_1_reg &= ~BITS_ACCEL_FS_SEL;//0xE7;
	accel_config_1_reg |= (s->base_state.accel_fullscale << BIT_POS_ACCEL_FS_SEL);
	result |= inv_icm20789_mems_write_reg(s, MPUREG_ACCEL_CONFIG, 1, &accel_config_1_reg);

	return result;
}

static int apply_accel_bw(struct inv_icm20789 * s)
{
	int result = 0;
	...
	result |= inv_icm20789_mems_read_reg(s, MPUREG_ACCEL_CONFIG_2, 1, &accel_config_2_reg);
	accel_config_2_reg &= ~BITS_DEC2_CFG;
	accel_config_2_reg &= ~BITS_A_DLPF_CFG;

	// Set DEC2 and A_DLPF_CFG
	if(inv_icm20789_get_chip_power_state(s) & CHIP_LP_ENABLE) // accel LP mode
	{
	...
	result |= inv_icm20789_mems_write_reg(s, MPUREG_ACCEL_CONFIG_2, 1, &accel_config_2_reg);

	return result;
}

static int apply_accel_bias(struct inv_icm20789 * s)
{
	int i, axis_sign;
	int rc = -1;
	...
	if(!s->accel_reg_bias_computed) {
		...
		rc = inv_icm20789_mems_read_reg(s, MPUREG_XA_OFFS_H, 2, d);
		accel_reg_otp[0] = ((uint16_t)d[0] << 8) | d[1];
		rc = inv_icm20789_mems_read_reg(s, MPUREG_YA_OFFS_H, 2, d);
		accel_reg_otp[1] = ((uint16_t)d[0] << 8) | d[1];
		rc = inv_icm20789_mems_read_reg(s, MPUREG_ZA_OFFS_H, 2, d);
	...
	} else if((s->power_state == PowerStateAccLNState) || (s->power_state == PowerState6AxisLNState) || (s->power_state == PowerState6AxisLPState)) {
		for(i = 0; i < 3; i++)
	...
	rc = inv_icm20789_mems_write_reg(s, MPUREG_XA_OFFS_H, 2, d);

	d[0] = (sensor_bias[1] >> 8) & 0xff;
	d[1] = sensor_bias[1] & 0xff;
	rc |= inv_icm20789_mems_write_reg(s, MPUREG_YA_OFFS_H, 2, d);

	d[0] = (sensor_bias[2] >> 8) & 0xff;
	d[1] = sensor_bias[2] & 0xff;
	rc |= inv_icm20789_mems_write_reg(s, MPUREG_ZA_OFFS_H, 2, d);

	return rc;
}

static int apply_gyr_fsr_bw(struct inv_icm20789 * s)
{
	int result = 0;
	uint8_t data;
	...
	// Set FSR
	result |= inv_icm20789_mems_read_reg(s, MPUREG_GYRO_CONFIG, 1, &gyro_config_1_reg);
	gyro_config_1_reg &= ~BITS_GYRO_FS_SEL;
	gyro_config_1_reg |= (inv_icm20789_get_gyro_fullscale(s)<< BIT_POS_GYRO_FS_SEL);
	result |= inv_icm20789_mems_write_reg(s, MPUREG_GYRO_CONFIG, 1, &gyro_config_1_reg);
	...
	result |= inv_icm20789_mems_read_reg(s, MPUREG_LP_CONFIG, 1, &data);
	data &= ~BITS_G_AVGCFG;
	data |= (g_avgcfg<<BIT_POS_G_AVGCFG);
	result |= inv_icm20789_mems_write_reg(s, MPUREG_LP_CONFIG, 1, &data);

	// Set DLPF_CFG
	result |= inv_icm20789_mems_read_reg(s, MPUREG_CONFIG, 1, &data);
	data &= ~BITS_DLPF_CFG; // Clear bits[2:0]
	data |= inv_icm20789_get_gyro_bandwidth(s);
	result |= inv_icm20789_mems_write_reg(s, MPUREG_CONFIG, 1, &data);

	return result;
}

static int apply_gyr_bias(struct inv_icm20789 * s)
{
	int i;
	int rc = -1;
	int sensor_bias[3];
	...
	rc = inv_icm20789_mems_write_reg(s, MPUREG_XG_OFFS_USRH, 2, data);

	data[0] = (sensor_bias[1] >> 8) & 0xff;
	data[1] = sensor_bias[1] & 0xff;
	rc |= inv_icm20789_mems_write_reg(s, MPUREG_YG_OFFS_USRH, 2, data);

	data[0] = (sensor_bias[2] >> 8) & 0xff;
	data[1] = sensor_bias[2] & 0xff;
	rc |= inv_icm20789_mems_write_reg(s, MPUREG_ZG_OFFS_USRH, 2, data);

	return rc;
}

int inv_icm20789_check_wom_status(struct inv_icm20789 * s, uint8_t int_status)
{
	int wom_data = 0;
...
int inv_icm20789_enable_mems(struct inv_icm20789 * s, int bit_mask, uint16_t smplrt_divider)
{
	int result = 0;
	...
	if ((bit_mask & GYRO_AVAILABLE) && (GYRO_LN_MODE == 1))
		bit_mask |= HIGH_ODR_REQUESTED;
	...
	case 0 :
		...
		result |= inv_icm20789_mems_write_reg(s, MPUREG_PWR_MGMT_1, 1, &s->base_state.pwr_mgmt_1);

		s->base_state.pwr_mgmt_2 |= BIT_PWR_ALL_OFF;
		result |= inv_icm20789_mems_write_reg(s, MPUREG_PWR_MGMT_2, 1, &s->base_state.pwr_mgmt_2);
	...
	case ACCEL_AVAILABLE: // to accel Low Power
		...
		switch(s->power_state) 
		{
			case PowerStateSleepState:
				...
				inv_icm20789_sleep(2);
				result |= apply_accel_fsr(s);

				s->base_state.pwr_mgmt_1 &= ~BIT_SLEEP;
#if ((MEMS_CHIP == HW_ICM20603) || (MEMS_CHIP == HW_ICM20789))
				...
				inv_icm20789_sleep(20); // accel startup time
				break;
	...
	case GYRO_AVAILABLE: // to gyro Low Power
		...
		switch(s->power_state) 
		{
			case PowerStateSleepState:
...
#endif
				result |= inv_icm20789_mems_write_reg(s, MPUREG_LP_CONFIG, 1, &s->base_state.lp_config);
				s->base_state.pwr_mgmt_2 |= BIT_PWR_ACCEL_STBY;
				s->base_state.pwr_mgmt_2 &= ~BIT_PWR_GYRO_STBY;
				result |= inv_icm20789_mems_write_reg(s, MPUREG_PWR_MGMT_2, 1, &s->base_state.pwr_mgmt_2);
				s->base_state.pwr_mgmt_1 &= ~BIT_SLEEP;
				...
				inv_icm20789_sleep(50); // gyro startup time
				break;
			case PowerStateAccLPState:
				//Procedure 19
#if (MEMS_CHIP == HW_ICM20603)
...
typedef	struct {
	...
	enum inv_icm20789_sensor InvSensor;
} MinDelayGenElementT;

/******************/

static const MinDelayGenElementT MinDelayGenList[]	= {
	/*{ ANDROID_SENSOR_ACCELEROMETER, INV_ICM20789_SENSOR_ACCELEROMETER, },*/
	{ ANDROID_SENSOR_GYROSCOPE, INV_ICM20789_SENSOR_GYROSCOPE, },
	{ ANDROID_SENSOR_RAW_ACCELEROMETER, INV_ICM20789_SENSOR_RAW_ACCELEROMETER, },
	{ ANDROID_SENSOR_RAW_GYROSCOPE, INV_ICM20789_SENSOR_RAW_GYROSCOPE, },
	{ ANDROID_SENSOR_GYROSCOPE_UNCALIBRATED, INV_ICM20789_SENSOR_GYROSCOPE_UNCALIBRATED, },
	{ ANDROID_SENSOR_GAME_ROTATION_VECTOR, INV_ICM20789_SENSOR_GAME_ROTATION_VECTOR, }
};
...
static MinDelayT MinDelayGenActual(struct inv_icm20789 * s, const MinDelayGenElementT * Element, u32 ElementQuan)
{
	MinDelayT MinDelay = (MinDelayT)-1;

	while(ElementQuan--) {
		if(is_icm20789_sensor_enabled(s, Element->InvSensor)) {
...
/******************/
static int set_hw_smplrt_dmp_odrs(struct inv_icm20789 * s, uint16_t * resulting_divider)
{
	int result = 0;
	uint8_t data = 0;
	...
	{
		result |= inv_icm20789_mems_read_reg(s, MPUREG_ACCEL_CONFIG_2, 1, &data);
	...
	{
		result |= inv_icm20789_mems_read_reg(s, MPUREG_GYRO_CONFIG, 1, &data);
		data &= ~BITS_FCHOICE_B; // Clear FCHOICE_B
		result |= inv_icm20789_mems_write_reg(s, MPUREG_GYRO_CONFIG, 1, &data);
	   ...
	   (s->enabled_sensor_mask & INV_NEEDS_GYRO_MASK) )
	{
		...
		if (minDly != 0xFFFF)
		{
			...
			dmp_icm20789_set_sensor_rate(s, INV_SENSOR_ACCEL, 200/(1000/minDly)-1);
			dmp_icm20789_set_sensor_rate(s, INV_SENSOR_GYRO, 200/(1000/minDly)-1);
			dmp_icm20789_set_sensor_rate(s, INV_SENSOR_SIXQ, 200/(1000/minDly)-1);
		}
	}
...
int inv_icm20789_set_sensor_period(struct inv_icm20789 * s, enum inv_icm20789_sensor sensor, uint16_t delayInMs)
{
	int result = 0;
...
//	if(delayInMs > ODR_MAX_DELAY) delayInMs = ODR_MAX_DELAY;
	...
	result |= inv_icm20789_disable_dmp(s);
	result |= inv_icm20789_disable_fifo(s);
	result |= inv_icm20789_reset_fifo(s);
	...
	result |= inv_icm20789_enable_mems(s, (int)data_rdy_status, resulting_divider);

	// need to reset and restart FIFO with new configurations
	result |= inv_icm20789_enable_fifo(s);
	result |= inv_icm20789_enable_dmp(s);

	return result;
}

int inv_icm20789_is_sensor_enabled(struct inv_icm20789 * s, enum inv_icm20789_sensor sensor)
{
	if(sensor < INV_ICM20789_SENSOR_MAX) {
...
int inv_icm20789_enable_sensor(struct inv_icm20789 * s, enum inv_icm20789_sensor sensor, uint8_t enable)
{
	int result = 0;
	uint16_t data_rdy_status = 0;
	uint16_t resulting_divider = 0;
	...
	// enable DMP features
	if( (s->enabled_sensor_mask & (1<<INV_ICM20789_SENSOR_RAW_ACCELEROMETER)) /*|| 
	    (s->enabled_sensor_mask & (1<<INV_ICM20789_SENSOR_ACCELEROMETER))*/)
		data_output_control1_mask |= ACCEL_SET;
	
	if( (s->enabled_sensor_mask & (1<<INV_ICM20789_SENSOR_RAW_GYROSCOPE)) ||
	    (s->enabled_sensor_mask & (1<<INV_ICM20789_SENSOR_GYROSCOPE)) ||
		(s->enabled_sensor_mask & (1<<INV_ICM20789_SENSOR_GYROSCOPE_UNCALIBRATED)))
		data_output_control1_mask |= GYRO_SET;
		
	if( s->enabled_sensor_mask & (1<<INV_ICM20789_SENSOR_GAME_ROTATION_VECTOR) ) {
		data_output_control1_mask |= QUAT6_SET;
		data_output_control1_mask |= GYRO_SET;
	}
	
	result |= dmp_icm20789_set_data_output_control1(s, data_output_control1_mask);
	...
	result |= dmp_icm20789_set_motion_event_control(s, motion_event_control_mask);
...
#if 0
int inv_icm20789_configure_accel_wom(struct inv_icm20789 * s, uint8_t wom_threshold)
{
	int result = 0;
	uint8_t data;
	...
	if(wom_threshold) {
		// Set threshold
		// wom_threshold = 0x0F;
		result |= inv_icm20789_mems_write_reg(s, MPUREG_ACCEL_WOM_THR,   1, &wom_threshold);
		result |= inv_icm20789_mems_write_reg(s, MPUREG_ACCEL_WOM_X_THR, 1, &wom_threshold);
		result |= inv_icm20789_mems_write_reg(s, MPUREG_ACCEL_WOM_Y_THR, 1, &wom_threshold);
		result |= inv_icm20789_mems_write_reg(s, MPUREG_ACCEL_WOM_Z_THR, 1, &wom_threshold);

		// Enable wake on motion interrupt on all 3 axes
		result |= inv_icm20789_mems_read_reg(s, MPUREG_INT_ENABLE, 1, &data);
		data |= 0xE0;
		data &= ~BIT_DATA_RDY_INT_EN; // clear data ready interrupt
		result |= inv_icm20789_mems_write_reg(s, MPUREG_INT_ENABLE, 1, &data);
		...
		result |= inv_icm20789_mems_read_reg(s, MPUREG_ACCEL_INTEL_CTRL, 1, &data);
		data |= 0xC0; // Compare current sample with the previous sample.
		              // Set WoM interrupt on OR of xyz axes interrupt.
		result |= inv_icm20789_mems_write_reg(s, MPUREG_ACCEL_INTEL_CTRL, 1, &data);

		// Wake up chip and set to LP mode
		result |= inv_icm20789_mems_read_reg(s, MPUREG_PWR_MGMT_1, 1, &data);
		data &= ~BIT_SLEEP; // Clear sleep bit
		data |= BIT_CYCLE; // Enable accel cycling mode.
		result |= inv_icm20789_mems_write_reg(s, MPUREG_PWR_MGMT_1, 1, &data);

		// Enable accel
		result |= inv_icm20789_mems_read_reg(s, MPUREG_PWR_MGMT_2, 1, &data);
		data &= ~BIT_PWR_ACCEL_STBY; // Enable accel
		data |= BIT_PWR_GYRO_STBY; // Disable gyro.
		result |= inv_icm20789_mems_write_reg(s, MPUREG_PWR_MGMT_2, 1, &data);
		...
		result |= inv_icm20789_enable_fifo(s, (int)data_rdy_status);
...
int inv_icm20789_disable_dmp(struct inv_icm20789 * s)
{
	int result = 0;
	uint8_t reg;

	/* disable DMP */
	result |= inv_icm20789_mems_read_reg(s, MPUREG_USER_CTRL, 1, &reg);
	reg &= ~BIT_DMP_EN;
	result |= inv_icm20789_mems_write_reg(s, MPUREG_USER_CTRL, 1, &reg);
	inv_icm20789_sleep(5);

	return result;
}

int inv_icm20789_disable_fifo(struct inv_icm20789 * s)
{
	int result = 0;
	uint8_t reg;
...
int inv_icm20789_enable_dmp(struct inv_icm20789 * s)
{
	int result = 0;
	uint8_t reg;
...
int inv_icm20789_enable_fifo(struct inv_icm20789 * s)
{
	int result = 0;
	uint8_t reg;
...
int inv_icm20789_reset_dmp(struct inv_icm20789 * s)
{
	int result = 0;
	uint8_t reg;
...
int inv_icm20789_reset_fifo(struct inv_icm20789 * s)
{
	int result = 0;
	uint8_t reg;
...
int inv_icm20789_all_sensors_off(struct inv_icm20789 * s)
{
...
int inv_icm20789_poll_sensor(struct inv_icm20789 * s, void * context,
		void (*handler)(void * context, enum inv_icm20789_sensor sensor, uint64_t timestamp, const void * data, const void *arg))
{
	...
	float grv_float[4];
	float gyro_float[3];
	//static uint64_t lastIrqTimeUs = 0;
	...
	currentIrqTimeUs = inv_icm20789_get_time_us();
	do {

		/* Mirror FIFO contents and stop processing FIFO if an error was detected*/
		if (inv_icm20789_fifo_swmirror(s, &data_left_in_fifo, &total_sample_cnt, sample_cnt_array))
			break;
			
		// In ICM20789 DMP3, each sample has a header, vs in vanadium, each packet has one header (one packet contains all sensor data available at sampling) 
		// check sample count
//		sample_nb = 0;
...
//			lastIrqTimeUs = currentIrqTimeUs;
		...
		while(total_sample_cnt--) {
			...
			if (inv_icm20789_fifo_pop(s, &header, &header2, &data_left_in_fifo))
				break;
				
			/* Gyro sample available from DMP FIFO */
			if (header & GYRO_SET) {
				float lScaleDeg = (1 << inv_icm20789_get_gyro_fullscale(s)) * 250.f; // From raw to dps to degree per seconds
				...
				inv_icm20789_dmp_get_raw_gyro(short_data);  
				raw_data[0] = (long) short_data[0];
				raw_data[1] = (long) short_data[1];
				raw_data[2] = (long) short_data[2];
				inv_icm20789_convert_dmp3_to_body(s, raw_data, lScaleDeg/(1L<<15), gyro_raw_float);                            
					
				if(inv_icm20789_is_sensor_enabled(s, INV_ICM20789_SENSOR_RAW_GYROSCOPE)) {
					long out[3];
					inv_icm20789_convert_quat_rotate_fxp(s->s_quat_chip_to_body, raw_data, out);
					//s->timestamp[INV_ICM20648_SENSOR_RAW_GYROSCOPE] += s->sensorlist[INV_ICM20648_SENSOR_RAW_GYROSCOPE].odr_applied_us;
					handler(context, INV_ICM20789_SENSOR_RAW_GYROSCOPE, ts, out, &dummy_accuracy);
				}
					
				/* Read bias gyro out of DMP FIFO and convert it from raw data format to degrees per seconds */
				inv_icm20789_dmp_get_gyro_bias(bias_data);
				signed long scaled_bias[3];
				scaled_bias[0] = bias_data[0]/2859;
				scaled_bias[1] = bias_data[1]/2859;
				scaled_bias[2] = bias_data[2]/2859;

				inv_icm20789_convert_dmp3_to_body(s, scaled_bias, lScaleDeg_bias/(1L<<15), gyro_bias_float);
				...
				if(inv_icm20789_is_sensor_enabled(s, INV_ICM20789_SENSOR_GYROSCOPE)) {
					...
					inv_icm20789_convert_dmp3_to_body(s, long_data, lScaleDeg_bias/(1L<<30), gyro_float);
					handler(context, INV_ICM20789_SENSOR_GYROSCOPE, ts, gyro_float, &gyro_accuracy);
				}
				
				if(inv_icm20789_is_sensor_enabled(s, INV_ICM20789_SENSOR_GYROSCOPE_UNCALIBRATED)) {
					...
					handler(context, INV_ICM20789_SENSOR_GYROSCOPE_UNCALIBRATED, ts, raw_bias_gyr, &gyro_accuracy);
				}
			}
			...
			if (header & ACCEL_SET) {
				...
				inv_icm20789_dmp_get_accel(long_data);

				if(inv_icm20789_is_sensor_enabled(s, INV_ICM20789_SENSOR_RAW_ACCELEROMETER)) {
					long out[3];
					inv_icm20789_convert_quat_rotate_fxp(s->s_quat_chip_to_body, long_data, out);
					out[0] /= 1<<15;
					out[1] /= 1<<15;
					out[2] /= 1<<15;
					//s->timestamp[INV_ICM20648_SENSOR_RAW_GYROSCOPE] += s->sensorlist[INV_ICM20648_SENSOR_RAW_GYROSCOPE].odr_applied_us;
					handler(context, INV_ICM20789_SENSOR_RAW_ACCELEROMETER, ts, out, &dummy_accuracy);
				}
/*
				if(inv_icm20789_is_sensor_enabled(s, INV_ICM20789_SENSOR_ACCELEROMETER)) {
					float scale;
					accel_accuracy = inv_icm20789_get_accel_accuracy();
					scale = (1 << inv_icm20789_get_accel_fullscale(s)) * 2.f / (1L<<30); // Convert from raw units to g's

					inv_icm20789_convert_dmp3_to_body(s, long_data, scale, accel_float);

					handler(context, INV_ICM20789_SENSOR_ACCELEROMETER, ts, accel_float, &accel_accuracy);
				}
*/				
			}
			...
			if (header & QUAT6_SET) {
				...
				inv_icm20789_dmp_get_6quaternion(long_quat);
				if(inv_icm20789_is_sensor_enabled(s, INV_ICM20789_SENSOR_GAME_ROTATION_VECTOR)) {
					/* and convert it from Q30 DMP format to Android format only if GRV sensor is enabled */
					inv_icm20789_convert_rotation_vector(s, long_quat, grv_float);
					ref_quat[0] = grv_float[3];
					ref_quat[1] = grv_float[0];
					ref_quat[2] = grv_float[1];
					ref_quat[3] = grv_float[2];
					handler(context, INV_ICM20789_SENSOR_GAME_ROTATION_VECTOR, ts, ref_quat, &gyro_accuracy);
				}					
			}
		}
